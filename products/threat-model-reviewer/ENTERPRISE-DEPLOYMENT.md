...
*Applies to Threat Model Reviewer v2.1.1 and later.*
...
msiexec /i "ThreatModelReviewer-v2.1.1-x64.msi" ALLUSERS=1 /qn /norestart /l*v install.log

# Per-user (no elevation)
msiexec /i "ThreatModelReviewer-v2.1.1-x64.msi" ALLUSERS="" /qn /norestart
...
```powershell
ThreatModelReviewer-v2.1.1-setup.exe /VERYSILENT /NORESTART /SUPPRESSMSGBOXES /LOG="install.log"
```

### Portable ZIP

```powershell
Expand-Archive .\ThreatModelReviewer-v2.1.1-win-x64-portable.zip -DestinationPath 'C:\Program Files\ThreatModelReviewer'
...
2. **Install command**
   ```
   msiexec /i "ThreatModelReviewer-v2.1.1-x64.msi" ALLUSERS=1 /qn /norestart
   ```
3. **Uninstall command**
   ```
   msiexec /x "{7E2D9A14-3C5B-4F8E-A1D6-9B0C2E4F6A38}" /qn /norestart
...
5. **Detection rule:** MSI product code, or a file rule on
   `%ProgramFiles%\Threat Model Reviewer\ThreatModelReviewer.exe` with **version ≥ 2.1.1.0**.
...
```powershell
Get-AuthenticodeSignature .\ThreatModelReviewer-v2.1.1-x64.msi | Format-List Status, SignerCertificate
Get-FileHash .\ThreatModelReviewer-v2.1.1-x64.msi -Algorithm SHA256
```
